    ...
    <div class="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
      ...
      <div class="rounded-xl border bg-card">
        <DataTable>
          ...
          <DataTableBody>
            <DataTableRow v-for="store in stores.data" :key="store.id">
              <DataTableCell>
                <div class="flex items-start gap-3">
                  <div>
                    <div class="font-medium">{{ store.name }}</div>
                    <div class="text-sm text-muted-foreground">
                      {{ store.description ? store.description.substring(0, 50) + '...' : 'No description' }}
                    </div>
                  </div>
                </div>
              </DataTableCell>
              ...
              <DataTableCell>
                <div v-if="store.contact_email || store.contact_phone">
                  <div v-if="store.contact_email" class="text-sm">{{ store.contact_email }}</div>
                  <div v-if="store.contact_phone" class="text-sm text-muted-foreground">{{ store.contact_phone }}</div>
                </div>
              ...
              <DataTableCell>
                <span
                  ...
                  :class="store.is_active ? 'bg-green-500/10 text-green-500' : 'bg-yellow-500/10 text-yellow-500'"
                >
